...
const renderTabBar = (props, DefaultTabBar) => (
    <Sticky topOffset={-60}>
        {({ style }) => (
            <DefaultTabBar {...props} style={{ ...style, zIndex: 1, background: '#fafafa', boxShadow:"0 6px 10px 0 rgba(0,0,0,.04)", top: 60 }} />
        )}
    </Sticky>
);
...
class BasicLayout extends React.PureComponent {
    ...
    render() {
        ...
        return (
            <DocumentTitle
                title={`${this.getTabTitle(pathname)} - ${globalConfig.name}`}
            >
                <Layout>
                    <Header />
